- [Home](/)

- Using the Tool

  - [Quick Start](using/quick-start.md)
  - [Running Generators](using/running-generators.md)
  - [Command Options](using/command-options.md)
  - [Polaris Pipeline Composer](using/polaris-composer.md)
  - [Assumptions & Concepts](using/assumptions.md)
  - [OCI Artifacts](using/oci-artifacts.md)

- Generators

  - [Overview](using/generators.md)
  - [nr-repository-composer](using/generators/nr-repository-composer.md)
  - [backstage](using/generators/backstage.md)
  - [backstage-location](using/generators/backstage-location.md)
  - [gh-common-mono-build](using/generators/gh-common-mono-build.md)
  - [gh-docker-build](using/generators/gh-docker-build.md)
  - [gh-docs-deploy](using/generators/gh-docs-deploy.md)
  - [gh-issue-templates](using/generators/gh-issue-templates.md)
  - [gh-maven-build](using/generators/gh-maven-build.md)
  - [gh-nodejs-build](using/generators/gh-nodejs-build.md)
  - [gh-oci-deploy-ocp](using/generators/gh-oci-deploy-ocp.md)
  - [gh-oci-deploy-onprem](using/generators/gh-oci-deploy-onprem.md)
  - [ocp-knox-provision](using/generators/ocp-knox-provision.md)
  - [gh-polaris-composer-agent](using/generators/gh-polaris-composer-agent.md)
  - [gh-tomcat-deploy-onprem](using/generators/gh-tomcat-deploy-onprem.md)
  - [migrations](using/generators/migrations.md)

- Extending the Tool

  - [Architecture](extending/architecture.md)
  - [Adding a New Prompt](extending/adding-a-prompt.md)
  - [Adding a New Generator](extending/adding-a-generator.md)
  - [YAML Path Mappings](extending/yaml-mappings.md)
  - [Workflow Templates](extending/workflow-templates.md)
  - [Development Workflow](extending/development.md)
  - [Testing](extending/testing.md)

- Reference

  - [Admin Tools](reference/tools.md)
  - [License](reference/license.md)
