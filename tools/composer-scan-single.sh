...
#for REPO in $REPOS; do
    ...
    if [[ -n "$ANNOTATION_VALUE" && "$ANNOTATION_VALUE" != "null" ]]; then
        ...
        for VALUE in "${VALUES[@]}"; do
          ...
          docker run --rm -v ${PWD}:/src -u $(id -u):$(id -g) --entrypoint yo ghcr.io/bcgov/nr-repository-composer:latest nr-repository-composer:$VALUE --headless --force
          ...
          if [[ $? -ne 0 ]]; then
            ...
            if [[ -n "$(git status --porcelain)" ]]; then
              ...
              echo "git generate branch name: $BRANCH_NAME"
